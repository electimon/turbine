...
import threading, os, urllib, atexit, socket, re
...
class Ui_Djl(QtGui.QMainWindow, interface.Onglets_djl,  diff.Main, irc.interface_IRC, rss.UiRSS, depot.UiDepot, depot.Foncs_Depot, modules.Interface):    
    def filtre_liste(self):
        ...
        for i in range(self.nb_jeux):
            ...
            if (int(type_jeu) == int(filtre_genre)) | (int(filtre_genre) == 0):
                ...
                if (int(licence) == int(filtre_licence)) | (int(filtre_licence) == 0):
                    ...
                    #Si on a la version de développement, on ajoute un * au nom du jeu si il n'est pas validé, afin de le reconnaitre:
        ...
        self.label.setText(i18n.traduc("Liste des jeux disponible: (")  + str(len(self.liste_jeux_ok)) + '/' + str(self.nb_jeux) + ')')
    ...
    def maj_description(self):
        ...
        #Troule l'article associé au jeu (généralement sur jeuxlinux.fr) afin d'avoir plus d'informations sur celui-ci:
        ...
        #Si le jeu est en cours d'installation, on desactive les boutons d'installation/suppression.
        ...
        for i in range(len(liste_depot)):
            ...
            if index != 'nul':
                ...
                if not os.path.exists(adresse_icone):
                    ...
                    urllib.urlretrieve('http://djl.jeuxlinux.fr/images/logo/' + icone,  adresse_icone,  reporthook=False)
    ...
    def info(self):
        ...
        if 'desktop' in str(jeu_actuel):
            ...
            #On va chercher le lien vers le site officiel du jeu(uniquement si on est déjà connecté au webservice
            if self.connecte_ws == 1:
                try:
                    self.lien_site = self.detail_jeux(self.trouve_index(jeu_actuel), type_info = 'site') 
                    variables.lien_site = 1
                except socket.gaierror:
                    self.lien_site = "nul" #Si il ne peut pas se connecter au serveur, tant pis pour le lien
    ...
    def trouve_index(self,  dossier='',  id=0): 
        ...
        return 'nul' #Si il a fini la boucle et qu'on a rien trouvé, le jeu sera ignoré faute d'informations.
...
#            return self.parse_listejeux(i, [self.trouve_index(nom_jeu)], 0)
    ...
    def verif_dependances(self):
        ...
        if os.path.exists(repertoire + '/lib'):
            LIBRARY_PATH = repertoire + ':' + repertoire + '/lib:' + config(info=2) + "/libs" + ':/usr/lib'
        elif os.path.exists(repertoire + '/libs'):
            LIBRARY_PATH = repertoire + ':' + repertoire + '/libs:' + config(info=2) +"/libs" + ':/usr/lib'
        else:
        ...
        if len(retour) > 2:
            for i in range(len(retour)):
                if "not found" in retour[i]: #Si il semble manquer la librairie, on l'ajoute dans la chaine des dependances manquantes
                    trouve = True
    ...
    def lance(self, Qitem='', Djeu='', Darguments=''):
        ...
        except: pass #Mieux vaut ignorer les exception, sinon on ne peut plus lancer les jeux
    ...
    def installer_jouer(self):
        ...
        #On fini le nettoyage pour récupérer le nom de l'archive (exemple: file=wings2_v1.3.6_linux.tar.gz)
        if "=" in nom_archive:
        ...
        if self.etat_actuel == 0:
            if adresse_telecharge != 0:
                ...
                icone = config(info=2) + '/' + config(info=14)+ '/icos/' + self.detail_jeux(no_description, type_info = 'icone')
...
#                self.vliste = False
        ...
        #Si le fichier de débogage a changé, on affiche une boite avec le message d'erreur (mais il peut être filtré) (voir classe msg_erreur(QtGui.QWidget) dans installe.py)
   ...
   #def ech_fenetre(self):
    ...
    def __init__(self,  app):
        ...
        if variables.type_maj == 1:
            ...
            diff.ecrit_historique(i18n.traduc("Impossible d'ecrire dans le repertoire de djl, il ne sera donc pas mis a jour."))
