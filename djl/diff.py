   ...
   # Liste globale pour définir la liste des jeux installés:
    ...
    def fonc_wine(self):
        ...
        if os.path.exists('/usr/bin/wine') or os.path.exists('/usr/local/bin/wine') or os.path.exists(home + '.local/bin/wine'):
            ...
            interface.info_box(
                i18n.traduc("Le logiciel Wine n'a pas ete trouve, veuillez l'installer pour continuer."))
    ...
    def recherche_internet_jeu(self):
        ...
        if " " in jeu_actuel:
            jeu_actuel = jeu_actuel.replace(" ", "+")
        lien = "http://www.exalead.com/search/results?q=" + \
            jeu_actuel + "&sourceid=djl-linux&$rcexpanded=true"
        ...
        if nom_icone == "":
            ...
            if os.path.exists(adresse + nom_icone) == False or self.entete_fichier(adresse + nom_icone) == 1:
               ...
               # print "Icone téléchargé"
                ...
                # Si l'icone existe mais qu'elle fait moins que 1Ko, on force le
    ...
    def affiche_image(self, no_description=-1):
        ...
        if os.path.exists(adresse + nom_image) == False or self.entete_fichier(adresse + nom_image) == 1:
           ...
           # print "Image téléchargé"
            ...
            # Si l'image existe mais qu'elle fait moins que 10Ko, on force le
            # téléchargement d'une nouvelle
    ...
    def verif_maj_jeu(self, nom_jeu):
        ...
        try:
            ...
            # Si il ne trouve pas la version parce que le fichier n'existe pas, on le
            # créé depuis la version dans le fichier de définition
...
#            return
    ...
    def barre_icone(self):
        ...
        if QtGui.QSystemTrayIcon.isSystemTrayAvailable() == True:
            ...
            self.tray.connect(
                ...
                self.clique_icone_barre)

            self.action_Fenetre_principale.connect(
                self.action_Fenetre_principale,
                QtCore.SIGNAL("triggered()"),
                self.clique_icone_barre)

            if interface == "1":
                self.a_affiche_actus.connect(
                    self.a_affiche_actus,
                    QtCore.SIGNAL("triggered()"),
                    self.affiche_actus)
                self.a_affiche_jeux.connect(
                    self.a_affiche_jeux,
                    QtCore.SIGNAL("triggered()"),
                    self.affiche_liste_jeux)
                self.a_affiche_depot.connect(
                    self.a_affiche_depot,
                    QtCore.SIGNAL("triggered()"),
                    self.affiche_depot)
                self.a_affiche_irc.connect(
                    self.a_affiche_irc,
                    QtCore.SIGNAL("triggered()"),
                    self.affiche_irc)
            ...
            self.action_Fenetre_principale.setText(
                i18n.traduc("Afficher/Masquer fenetre principale"))
            if interface == "1":
    ...
    def boite_infos(self):
        ...
        if ".desktop" in nom:
            ...
            # Si c'est un jeu Windows avec Wine...
                ...
                jeu_win = 1
            # Il faut filtrer à fond pour réussir à trouver le fichier .exe avec la
            # syntaxe des raccourcis Wine
                if "C:" in binaire:
        ...
        if raccourci == 1:
            ...
            # Dernier accès au fichier: (utiliser time.localtime pour utiliser la date
            # sous forme de liste)
            ...
            texte = texte + "\n" + \
                i18n.traduc("Espace utilise") + ": "  + \
                self.espace_utilise(repertoire) + " " + i18n.traduc("Mo.")
    ...
    def verif_maj_djl(self, notif=0, serveur=""):
        ...
        # Si on tombe sur une erreur 404 ou 403 dans le fichier de version, on
    ...
    def compare_maj(self, version, nouvelle_version, notif, serveur):
        ...
        # Créé une boucle pour faire de la liste des changements une lignes de
        # caractères avec sauts de lignes:
        changements = ""
        ...
        if self.changements != '' or self.changements != '0':
            ...
            while boucle_ < len(self.changements):
                ...
                # Sinon, continue de chercher dans la boucle.
                    ...
                    # Variable qui défini le nombre maximum de versions à afficher dans
                    # l'historique des changements dans la boite de dialogue:
                    # (La dernière ligne sera "...", on affichera donc en réalité var_ch-1)
                    var_ch = 5
                    if boucle_ != var_ch:
        ...
        # Si la variable qui contient le numéro de version sur internet n'est pas
        # la même que la version actuelle, on affiche la boite de dialogue.
        if nouvelle_version != '0':
            if nouvelle_version != version:
                texte = i18n.traduc("Une mise a jour est disponible: ")
                reponse = QtGui.QMessageBox.information(
                    self,
                    titre,
                    texte + str(version) + ' -> ' + str(nouvelle_version) + txt_cg + changements,
        ...
        # Si sur le serveur la version est "0", on ne met pas à jour (état
        # transitoire vers une future mise à jour))
        else:
            if notif == 0:
                texte = i18n.traduc("djl est a jour: ")
                QtGui.QMessageBox.information(self, titre, texte + str(version))
    ...
    def affiche_maj(self, serveur, id_archive):
        ...
        # Ecrit dans un fichier la taille et la position de la fenêtre principale.
        self.writeSettings()
    ...
    def liste_raccourcis(self):
        ...
        for i in range(len(liste_raccourcis)):
            # Si le nom du fichier contient '~', c'est que c'est un doublon qui a été
            # modifié par un éditeur de texte, on ne l'affiche pas.
            if not '~' in liste_raccourcis[i]:
                item = QtGui.QListWidgetItem(self.listWidget)
                icone = str(
                    self.lit_fichier_desktop(fichier=liste_raccourcis[i],
                                             type_info='Icon'))
    ...
    def confirmation_suppr_desk(self):
        ...
        titre = i18n.traduc("Confirmation")
        texte = i18n.traduc("Voulez vous vraiment supprimer le raccourci") + \
            " " + jeu_actuel + " ?"
        reponse = QtGui.QMessageBox.information(
            self,
            titre,
                ...
                if 'Generic' in txt_raccourcis[boucle] or '[' in txt_raccourcis[boucle] and type_info == 'Name':
                    ...
                    if type_info == 'Icon':
                        if '/' in txt_raccourcis[boucle]:
                            ...
                            if self.djl_lance == 0:
                                ecrit_historique(
                                    _('icone introuvable:') + nom_fichier + i18n.traduc(", djl va tenter de la trouver, sinon elle sera remplacee."))

                            if retour == "":
                                ecrit_historique(
                                    i18n.traduc(
                                        "L'icone du raccourcis") +
                                    nom_fichier +
                                    i18n.traduc(
                                        " n'a pas ete trouve, elle a ete remplace par l'icone de djl."))
                                retour = dossier_racine + '/icone.png'
                                return retour

                            # Tente de trouver l'icone avec différentes adresse et différents types
                            # de fichiers:
                    ...
                    if type_info == 'Path':
                        if '$HOME' in txt_raccourcis[boucle]:
                            txt_raccourcis[boucle] = txt_raccourcis[boucle].replace(
                                '$HOME',
                                os.path.expanduser('~'))
    ...
    def infos_sys(self):
        ...
        texte_1 = i18n.traduc("Systeme d'exploitation: ") + \
            (self.retour_commande(commande="uname -o", filtre1='', filtre2=''))

        texte_2 = i18n.traduc("Nom de la machine: ") + \
            (self.retour_commande(commande="uname -n", filtre1='', filtre2=''))

        texte_3 = i18n.traduc("Architecture: ") + \
            (self.retour_commande(commande="uname -m", filtre1='', filtre2=''))

        texte_4 = i18n.traduc(
            "Environnement:  ") + (self.retour_commande(commande="echo $DESKTOP_SESSION",
                                                        filtre1='',
                                                        filtre2=''))

        texte_5 = i18n.traduc(
            "Processeur: ") + (self.retour_commande(commande="cat /proc/cpuinfo",
                                                    filtre1='model name',
                                                    filtre2=':'))

        texte_6 = i18n.traduc(
            "Fabricant de la carte graphique: ") + (self.retour_commande(commande="glxinfo",
                                                                         filtre1='OpenGL vendor string:',
                                                                         filtre2=''))

        texte_7 = i18n.traduc(
            "Nom de la carte graphique: ") + (self.retour_commande(commande="glxinfo",
                                                                   filtre1='OpenGL renderer string:',
                                                                   filtre2=''))

        texte_8 = i18n.traduc(
            "Version du pilote graphique: ") + (self.retour_commande(commande="glxinfo",
                                                                     filtre1='OpenGL version string:',
                                                                     filtre2=''))

        texte_9 = i18n.traduc(
            "Memoire vive: ") + str(int(self.retour_commande(commande="cat /proc/meminfo",
                                                             filtre1='MemTotal:',
                                                             filtre2='kB')) / 1024) + " " + _('Mo') + "\n"

        texte_10 = i18n.traduc(
            "Carte son : ") + (self.retour_commande(commande="cat /proc/asound/card0/id",
                                                    filtre1='',
                                                    filtre2=''))
    ...
    def rapport(self):
        ...
        # self.lance_navigateur("http://forum.jeuxlinux.fr/viewforum.php?id=26")
        # #Forum jeuxlinux.fr
    ...
    def a_propos(self):
        ...
        texte_1 = i18n.traduc("Depot jeux linux ") + interface.version() + \
            i18n.traduc(" par Diablo150 sous licence GPL 3.")
        ...
        texte = texte_1 + "<br />" + texte_2 + "<br />" + texte_3 + \
            "<br />" + texte_4 + "<br />" + texte_5 + "<br />" + texte_6
    ...
    def retour_commande(self, commande='', filtre1='', filtre2=''):
        ...
        while boucle < len(retour):
            # Fais le tri dans la liste afin de ne récupérer que ce qui contient
...
class Ui_Telecharge_Lib(QtGui.QWidget):
    ...
    def __init__(self, parent=None, serveur='', repertoire='', titre=''):
        ...
        # self.serveur = serveur + "/maj_djl/" + repertoire #Serveur pour
        # télécharger les librairies
        # Vérifi ou non si les librairies n'existent pas dejà dans /usr/lib
        self.verif_en_dur = False

        self.titre = titre  # Titre de la fenêtre

        # Liste des fichiers, sera rempli en fonction des éléments de la QlistWidget
        self.liste = []
    ...
    def affiche_liste(self):
        ...
        for i in range(len(liste)):
            ...
            if not os.path.exists("/usr/lib/" + liste[i]) or not self.verif_en_dur:
                # Si c'est la fenêtre des dependances, les fichiers et la description sont
                # les mêmes
    ...
    def installe_supprime(self):
        ...
        if fichier != 'titre':
            ...
            # Si elle n'existe pas, on la télécharge
                ...
                # Change l'icone dans la liste pour montrer qu'elle pourra maintenant être
                # supprimée
                icone = QtGui.QIcon(dossier_racine + '/res/retirer.png')
...
class Ui_Maj(QtGui.QWidget):
    ...
    def timerEvent(self, event):
        ...
        if int(variables_maj.pourcentage) >= 100:
            ...
            # Si on a les droits dans le répertoire de djl, on y décompresse le programme
            if variables.type_maj == 0:
    ...
    def setupUi(self, Form):
        ...
        Form.setObjectName("Form")
        Form.resize(QtCore.QSize(QtCore.QRect(0, 0, 281, 77).size())
                    .expandedTo(Form.minimumSizeHint()))
...
class th_telechargement_maj(threading.Thread):
    ...
    def reporthook(self, nb_blocs, taille_bloc, taille_fichier):
        ...
        variables_maj.pourcentage = int(
            (((nb_blocs + 0.) * (taille_bloc + 0.)) / taille_fichier + 0.) * 100 + 0.)
...
class Ui_Journal(QtGui.QWidget):
    ...
    def setupUi(self, Form):
        ...
        self.resize(QtCore.QSize(QtCore.QRect(0, 0, 800, 600).size())
                    .expandedTo(Form.minimumSizeHint()))
...
class redemarre_djl(threading.Thread):
    ...
    def run(self):
        ...
        elif os.path.exists("/usr/bin/python2.6") == True:
...
class MessageLancement(QtGui.QWidget):
    ...
    def setupUi(self, fen):
        ...
        self.resize(QtCore.QSize(QtCore.QRect(0, 0, x, y).size())
...
class Ui_Wine(QtGui.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.wine_parcouir_icone.connect(
            self.wine_parcouir_icone,
            QtCore.SIGNAL("clicked()"),
            self.parcours_icone)
        self.wine_parcouir_exe.connect(
            self.wine_parcouir_exe,
            QtCore.SIGNAL("clicked()"),
            self.parcours_exe)
    ...
    def parcours_exe(self):
        ...
        commande = QtGui.QFileDialog.getOpenFileName(
            self,
            "",
            home + '/.wine/drive_c' '',
            i18n.traduc("Win32 exe (*.exe)"))
    ...
    def bouton_ok(self):
        ...
        if self.wine_nom_jeu.text() == '' or self.wine_chemin_exe.text() == '':
            interface.info_box(
                i18n.traduc("Vous devez definir le chemin vers un fichier .exe ainsi que le nom du jeu."))
        else:
            # Enregistre les données dans un fichier .desktop standard:
            fichier = open(
                config(info=2) + '/raccourcis/' +
                str(self.wine_nom_jeu.text()).lower() + '.desktop',
                'w')
            ...
            # Formatage de chaine pour utilise Wineprefix (si le .exe est placé dans
            # ~/.wine ou un sous rpéertoire:
    ...
    def setupUi(self, Wine, parent):
        ...
        Wine.resize(QtCore.QSize(QtCore.QRect(0, 0, x, y).size())
                    .expandedTo(Wine.minimumSizeHint()))
        ...
        self.wine_buttonBox.setStandardButtons(
    ...
    def retranslateUi(self, Wine):
        ...
        self.wine_label.setText(
            i18n.traduc("Attention: La stabilitee et les performances du jeu ne sont pas\nassures"))
