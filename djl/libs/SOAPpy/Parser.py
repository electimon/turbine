...
# XXX What to do here? (None, kind) is just going to fail in convertType
            ...
            if attrs:
                ...
                if null:
                    if len(cur) or \
                            (self._data is not None and string.join(self._data, "").strip() != ''):
...
# XXX What if rule != kind?
            ...
            if len(self._stack) == 3 and kind is None and \
                len(cur) == 0 and \
                    (self._data is None or string.join(self._data, "").strip() == ''):
                data = structType(name=(ns, name), attrs = attrs)
                break
    ...
    def convertToBasicTypes(self, d, t, attrs, config=Config):
        ...
        if t[0] in NS.EXSD_L:
            ...
            if self.intlimits.has_key(t[1]):  # integer types
                ...
                if l[1] is not None and d < l[1]:
                    raise UnderflowError, "%s too small" % d
                if l[2] is not None and d > l[2]:
