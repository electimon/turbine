...
class Foncs_Depot(object):
    ...
    def parse_listegenre(self, id, val=0):
        ...
        # Si val vaut 0, on renvoi l'id du type de jeu, si il vaut 1, on renvoi la
        # chaine de caractère.
    ...
    def parse_detailjeux(self, no_jeu, val=0):
        ...
        # Si les infos ont déjà été demandés, on ne re-télécharge pas les informations.
        if self.n_jeu_courant != no_jeu:
            try:
                self.detailjeux = self.clientSOAP.detailJeux(
                    self.parse_listejeux(no_jeu, 6), int(self.lang))
            except:
                # Si on ne trouve pas le jeu en dépot dans la langue séléctionnée, on
                # prend l'Anglais:
    ...
    def connexion_SOAP(self):
        ...
        # On pré-télécharge dans un Thread les images afin de rendre la navigation
        # dans le dépot plus rapide.
    ...
    def charge_depot(self):
        ...
        if os.path.exists(nom_fichier):
            self.connecte_ws = 0
            # Si la methode est à 1, on utilise cPickle pour charger le dépôt
            if self.methode_depot == 1:
                ...
                try:
                    # <<< Dans une liste, sinon ça déconne avec le nouveau ws
                    self.liste_jeux = [cPickle.load(fichier)]
                except EOFError:
                    fichier.close()
            ...
            # Si methode est à 2, on charge le dépôt avec la methode faite maison
            # (plus rapide, mais peut être plus boguée)
            else:
                fichier = open(nom_fichier, 'r')
...
class UiDepot(object):
    ...
    def StopTelechargement(self):
        ...
        # Stop le thread dédié au téléchargement:  (une exception est levée si le
        # téléchargement n'a pas encore commencé, dans ce cas, on passe)
        try:
    ...
    def NouvTelechargement(self, adresse_telecharge, nom_archive,
        ...
        # Ajoute au dictionnaire qui référence les téléchargements
        variables.installe[nom_jeu] = False

        # Fais clignoter la barre du depot, que l'utilisateur voit que le téléchargement est lancé
        self.clignote_depot = True
        self.Cltimer()

        # Envoi le téléchargement:
        th_t = Telechargement(adresse_telecharge, nom_archive, nom_jeu, commande, version)
        th_t.start()
        # Ajoute l'instance du téléchargement dans la liste ainsi que le nom de l'archive
        self.listeTelechargement.append((th_t, nom_archive, nom_jeu, titre))
    ...
    def SetupUi_Depot(self):
        ...
        if WebKit:  # Si QtWebkit est disponible...
            ...
            self.texte_description.setTextInteractionFlags(
    ...
    def connexionsDepot(self):
        ...
        # Attention, self.filtre_liste est ici executé au démarrage, quand la
        # combobox s'initialise:
        self.comboBox.connect(
            self.comboBox,
            QtCore.SIGNAL("currentIndexChanged(int)"),
            self.filtre_liste)
        self.combo_license.connect(
            self.combo_license,
            QtCore.SIGNAL("currentIndexChanged(int)"),
            self.filtre_liste)
        self.widget_liste_jeux.connect(
            self.widget_liste_jeux,
        ...
        self.boutton_installer.connect(
            self.boutton_installer,
            QtCore.SIGNAL("clicked()"),
            self.installer_jouer)
        self.boutton_supprimer.connect(
...
class sauvegarde_depot(threading.Thread):
    ...
    def __init__(self, liste, methode=1):
        ...
        # Si c'est à 2, on enregistre dans un format fait maison, rapide mais peut
        # être plus bogué que Pickle
