...
class Onglets_djl(object):
    ...
    def setupUi(self, MainWindow):
        ...
        MainWindow.resize(QtCore.QSize(QtCore.QRect(0, 0, x, y).size())
                          .expandedTo(MainWindow.minimumSizeHint()))
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.actionAjouter_jeu.setText(
            i18n.traduc("Ajouter un raccourci vers un jeu installe sur le systeme"))
        if config(info=10) == '1':
        ...
        self.action_historique.setText(
            i18n.traduc("Consulter le journal des modifications de djl"))
    ...
    def connexions_o(self):
        ...
        self.action_change_ui.connect(
            self.action_change_ui,
            QtCore.SIGNAL("triggered()"),
            self.change_ui)
        self.action_maj.connect(
            self.action_maj,
            QtCore.SIGNAL("triggered()"),
            self.trouve_serveur_maj)
        self.actionAjouter_jeu.connect(
            self.actionAjouter_jeu,
            QtCore.SIGNAL("triggered()"),
            self.ajout_jeu_)
        self.action_Import_desk.connect(
            self.action_Import_desk,
            QtCore.SIGNAL("triggered()"),
            self.import_desk)
        #self.actionSupprimer_desk.connect(self.actionSupprimer_desk, QtCore.SIGNAL("triggered()"), self.suppr_desk)
        self.actionInstallWine.connect(
            self.actionInstallWine,
            QtCore.SIGNAL("triggered()"),
            self.fonc_wine)
        self.actionConfig.connect(
            self.actionConfig,
            QtCore.SIGNAL("triggered()"),
            self.configuration)
        ...
        self.action_Infos_sys.connect(
            self.action_Infos_sys,
            QtCore.SIGNAL("triggered()"),
            self.infos_sys)
        self.action_historique.connect(
            self.action_historique,
            QtCore.SIGNAL("triggered()"),
            self.affiche_historique)
        self.action_journal.connect(
            self.action_journal,
            QtCore.SIGNAL("triggered()"),
            self.affiche_journal)
        self.action_sortie_jeu.connect(
            self.action_sortie_jeu,
            QtCore.SIGNAL("triggered()"),
            self.affiche_sortie_jeu)
        self.action_rapport.connect(
            self.action_rapport,
            QtCore.SIGNAL("triggered()"),
            self.rapport)
        self.action_Apropos.connect(
            self.action_Apropos,
            QtCore.SIGNAL("triggered()"),
            self.a_propos)
        ...
        self.action_creer_def.connect(
            self.action_creer_def,
            QtCore.SIGNAL("triggered()"),
            self.crer_entree)
        self.action_dependances.connect(
            self.action_dependances,
            QtCore.SIGNAL("triggered()"),
            self.fenetre_dependances)
        ...
        QtGui.QShortcut(
            QtGui.QKeySequence(QtCore.Qt.CTRL | QtCore.Qt.Key_5),
            self,
            self.chg_modules)

        QtGui.QShortcut(
            QtGui.QKeySequence(QtCore.Qt.CTRL | QtCore.Qt.Key_Left),
            self,
            self.chg_onglet_g)
        QtGui.QShortcut(
            QtGui.QKeySequence(QtCore.Qt.CTRL | QtCore.Qt.Key_Right),
            self,
            self.chg_onglet_d)

        if config(info=10) == '1':
            # On change le raccourcis avec la touche entrée suivant que l'on soit sur
            # la liste des jeux ou le canal IRC
            self.tabWidget.connect(
                self.tabWidget,
                QtCore.SIGNAL("currentChanged(int)"),
                self.ch_tab)

            # Appuyer sur la touche entrée dans la fenêtre principale reviendra soit à
            # envoyer un message sur IRC, soit à lancer un jeu
    ...
    def contextMenuEvent(self, event):
        ...
        if not self.int_etendue or var_ok == 1:  # Si c'est l'interface simple...
            ...
            if raccourci == 0:  # Si c'est un jeu installé en dur, on a le menu complet
                ...
                if os.uname()[4] != 'x86_64' or os.path.exists('/usr/bin/ldd32'):
                    self.act_verif_depend = QtGui.QAction(
                        _((i18n.traduc("Verifier les dependances"))),
                        self)
                    self.act_verif_depend.connect(
                        self.act_verif_depend,
                        QtCore.SIGNAL("triggered()"),
...
class Ui_Arguments(QtGui.QWidget):
    ...
    def setupUi(self, Form, parent):
        ...
        x, y = 400, 90
        Form.resize(QtCore.QSize(QtCore.QRect(0, 0, x, y).size())
