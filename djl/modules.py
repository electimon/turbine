...
import urllib
import sys
# Charge à chaud les modules sous forme de source Python brutes ou d'archives zip
import imp
import zipimport
import zipfile
...
class ModulesEvents(object):
    ...
    def EntreeMod(self):
        ...
        if self.tabModules.currentIndex() >= nombre_onglets:
            try:
                # On tente de lancer la fonction 'entree' du module, si elle existe pas,
                # c'est sans effet
    ...
    def EchapMod(self):
        ...
        if self.tabModules.currentIndex() >= nombre_onglets:
            try:
                # On tente de lancer la fonction 'echap' du module, si elle existe pas,
                # c'est sans effet
...
class GModules(object):
    ...
    def initGMods(self):
        '''Appelé au démarrage'''
        # Défini le conteneur pour les modules (Utilisé pour les gérer , mais le
        # vrai conteneur est défini dans chacun des modules):
    ...
    def MajMod(self):
        ...
        # self.self.liste_modules[i][0] #Liste des modules lancés
        # [no_modules][instance, Qicone, version]
    ...
    def import_modules(self):
        ...
        # pour faire le nettoyage. On utilise ensuite del pour délier en mémoire,
        # le module sera vidé de la mémoire.
        for i in range(len(self.liste_modules)):
            try:
                try:
        ...
        if os.path.exists(self.repMod):
            ...
            # Tri par ordre alphabétique les modules, c'est important, ça défini l'ordre d'import.
            liste.sort()
            for i in range(len(liste)):
                icone = QtGui.QIcon(dossier_racine + '/res/modules_oxygen.png')
                ext = liste[i].split(".")
                # si c'est bien un fichier python, on l'importe:
                # si il y a '-mod", c'est que le module est un répertoire
                if ext[len(ext) - 1] == "py" or "-mod" in liste[i] or ext[len(ext) - 1] == "zip":
                    ...
                    try:
                        # Si c'est un module sous forme de fichier .py ou autre
                        if type == 0 or type == 2:
                            try:
                                module = imp.load_source(nom_module, self.repMod + "/" + liste[i])
                            # Le problème d'importation vient du code du module, pas du chargement
                            # du module mêmeasurementSystem
                            ...
                            try:
                                ...
                                # Ouvre l'archive zip contenant les sources
                                fichier_zip = zipfile.ZipFile(archive, 'r')
                                # Récupère la liste des fichiers de l'archive
                                liste_fichiers = fichier_zip.namelist()

                                for id in range(len(liste_fichiers)):
                                    # Si le paquet contient une icone, on l'affiche
                                    if liste_fichiers[id] == "icone.png":
                                        # print "icone: ", liste_fichiers[id]
                                        # On extrait l'icone de l'archive...
                                        donn = fichier_zip.read(liste_fichiers[id])
                                        addr_ico = config(
                                            info=2) + '/' + config(info=14) + '/' + liste[i].split('.zip')[0] + '_' + liste_fichiers[id]
                                        # print len(donn)
                                        f = open(addr_ico, 'w')
                                        f.write(donn)
                                        f.close()
                                    ...
                                    if not '__init__' in liste_fichiers[id]:
                                        ...
                                        if '.py' in liste_fichiers[id]:
                                            zimp.load_module(
                                                liste_fichiers[id].replace('.py',
                                                                           ''))  # Précharge les sources (pas le script principal)

                                fichier_zip.close()
                                # Charge le fichier '__init__.py' du paquet zip.
                                module = zimp.load_module("__init__")
                            # Typiquement quand le fichier __init__ n'est pas trouvé
                            except (zipimport.ZipImportError), x:
                                print "Zip module error:", nom_module, '-', x
                                # On passe à l'itération de boucle suivante, le module ne sera pas
                                # chargé.
                                continue
                            #
                    except (NameError, AttributeError), x:
                        print nom_module, "Module error:", x
                        # Passe au module suivant si il y a un problème de dépendance avec celui là
                        continue

                    # Essai de trouver le numero de version du module, si ça plante (pas de
                    # numéro), on défini la version à -1
                    try:
                        versionMod = module.version
                    except AttributeError:
                        versionMod = '-1'

                    # Lance le module et ajoute son instance dans la liste (ça servira pour
                    # l'utiliser mais aussi pour le nettoyage).
                    try:
                        print module, versionMod  # , icone
                        # Lance le module et envoi l'instance dans la liste des modules (ainsi que
                        # l'icone, la version et le nom du fichier source)
                        self.liste_modules.append([module.main(self), icone, versionMod, liste[i]])
                        # sys.modules[nom_module].No = i #Numéro du module
                    except AttributeError, x:
                        # print "Plugin Error (import)> " + sys.modules[nom_module].nom_module +
                        # ": " + str(x)
...
class Interface(GModules, ModulesEvents):
    ...
    def Init_modules(self):
        ...
        # Nom du répertoire contenant les modules sur le serveur et dans la partie cliente
        repertoire = 'modules'
        ...
        # Serveur pour télécharger les modules
        self.srvMod = self.serveur_maj[0] + "/maj_djl/" + repertoire
...
#        self.timerMod.start(100, self.tab_5)
    ...
    def listeModules(self):
        ...
        # Si on ne peut pas accéder à la liste, on quitte la fonction, la liste
        # des modules sera vide.
        except IOError:
            return
        ...
        # Si il tombe sur une page d'erreur 404, on quitte la fonction, la liste
    ...
    def AffListeMod(self):
        ...
        for i in range(len(liste)):
            ...
            # L'élément 0 sera le nom du fichier, l'autre sera la description
            mini_liste = liste[i].split(";")
            texte = mini_liste[1]
    ...
    def InstSupprMod(self):
        ...
        if fichier != 'titre':
            ...
            # Rafraichi le bouton de mis à jour du module actuellement sélectionné
            self.AffMajMod()
    ...
    def SupprMod(self, module):
        ...
        # On supprime également tous les fichiers qui commencent par le nom du
        # module (le modules, fichiers .pyc, log, etc...
        liste = os.listdir(self.repMod)
        for i in range(len(liste)):
            l_fichier = liste[i].split('.')
            # Fatalement le nom du module ne doit comporter qu'un '.', genre 'fichier.py'
            if l_fichier[0] == module.split('.')[0]:
