...
#Problème quand on utilise /quit, l'utilisateur la chaine "utilisateur déco..." ne s'affiche pas (mais est ajouté à la liste)
...
class interface_IRC(object):
    def setupUiIRC(self):
        ...
        self.listeW_canaux.connect(self.listeW_canaux, QtCore.SIGNAL("itemSelectionChanged()"), self.change_canal)
        self.pushButton_chat.connect(self.pushButton_chat, QtCore.SIGNAL("clicked()"), self.envoi_irc)
        self.Button_connect_chat.connect(self.Button_connect_chat, QtCore.SIGNAL("clicked()"), self.connexion_irc)
            
        #Pareil sur la liste des utilisateurs sur le canal IRC:
        self.listWidget_chat.connect(self.listWidget_chat, QtCore.SIGNAL("itemClicked(QListWidgetItem*)"), self.message_pv_irc)
    ...
    def Prepare_IRC(self):
        ...
        self.textEdit_chat.connect(self.textEdit_chat, QtCore.SIGNAL("textChanged()"), self.rafraichi_IRC)
    ...
    def recoi_msg_irc(self):
        ...
        texte, canal,  pseudo = variables.recoi_irc[0],  variables.recoi_irc[1],  variables.recoi_irc[2]
        #Si il y a le pseudo de l'utilisateur dans le message...
        if self.pseudo in texte[0]:
            couleur = "255, 0, 0"
            #On fait clignoter le client si l'utilisateur n'affiche pas le client ou que djl est minimisé.
        ...
        #Sinon c'est un message normal
            ...
            if self.info_13 == 1:
                ...
                couleur = "0, 0, 0"
            #couleur = "255, 255, 255"
            #Si on recoi un message quelconque et qu'on affiche pas le canal qui le recoi, on marque le canal en rouge:
            if self.canal_courant != canal:
                #print "Couleur: Rouge"
                self.change_couleur_canal(canal, "rouge")
        chaine = QtGui.QApplication.translate("MainWindow", "<span style='color: rgb(" + couleur + ");'>" + pseudo + ": "+ texte[0] +"</span>", None, QtGui.QApplication.UnicodeUTF8) + "\n"
    ...
    #Si envoi=1, ça veut dire que c'est l'utilisateur qui envoi le texte, on ne vérifi donc pas si il est sur le bon canal.
    ...
    def message_pv_irc(self):
        ...
        nom_utilisateur = variables.liste_utilisateurs[self.canal_courant][self.listWidget_chat.currentRow()+1]
        ...
        #Si il y est déjà, on le supprime
            ...
            if " " + nom_utilisateur in self.line_Edit_chat.text():
                self.line_Edit_chat.setText(self.line_Edit_chat.text().replace(" " + nom_utilisateur, " "))
            else:
                self.line_Edit_chat.setText(self.line_Edit_chat.text().replace(nom_utilisateur, " "))
    ...
    def envoi_irc(self):
        ...
        if self.line_Edit_chat.text() != '':
            ...
            if texte [0] != "/": #On envoi rien si c'est une commande
                ...
                chaine = i18n.encode("--><span style='color: rgb("+couleur+");'> " + str(self.pseudo) + ': ' +  texte +'\n' + "</span>")
    ...
    def ajout_utilisateur_irc(self):
        ...
        if not self.pseudo in nom_utilisateur:
            ...
            chaine = "--><span style='color: rgb(0, 255, 0);'> " + i18n.traduc("Utilisateur connecte") + ': ' + nom_utilisateur+'\n' + "</span>"
    ...
    def supprime_utilisateur_irc(self):
        ...
        chaine = "--><span style='color: rgb(187, 111, 61);'> " + i18n.traduc("Utilisateur deconnecte") + ': ' + nom_utilisateur+'\n' + "</span>"

        #print ">0", variables.liste_utilisateurs[canal]
        #Enlève l'utilisateur déconnecté de la liste:
        
        if canal == None: #Si l'utilisateur a quitté complètement IRC (commande /quit)
            ...
            for id in variables.liste_utilisateurs:
                if nom_utilisateur in variables.liste_utilisateurs[id]:
                    #On donne à la fonction d'envoi des message des notifications le nom du canal où l'utilisateur vient que faire /quit:
    ...
    def cree_liste_utilisateurs_irc(self, canal):
        ...
        #Problème, on affiche le nouvel utilisateur sur le canal courant et non celui qui vientde réélement recevoir l'utilisateur.
...
class DialogueServeurIRC(irclib.SimpleIRCClient):
    ...
    def on_pubmsg(self, connection, event):
        ...
        variables.recoi_irc = [event.arguments(), event.target(),  self.split_source(event.source())]
    ...
    def on_join(self, connection, event):
        ...
        #print "Join: Canal:",  str(event.target()),  "utilisateur:", self.split_source(event.source())
    ...
    def on_part(self, connection, event):
        ...
        #print "Part: Canal:",  str(event.target()),  "utilisateur:", self.split_source(event.source())  
