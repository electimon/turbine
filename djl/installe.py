...
import urllib, os, shutil, subprocess, time
import threading, tarfile, zipfile
...
class Telechargement(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            urllib.urlretrieve(self.lien_telechargement, filename=(config(info=2) + '/jeux/') + self.nom_archive, reporthook = self.reporthook)
        except Annule:
...
#
        ...
        if tar_gz == 1:
            ...
            #Attention, la fonction extractall n'est (apparemment) compatible qu'avec Python 2.5 ou supérieur
            try:
        ...
        #Si c'est un installateur .bin, utilise un shell, avec les options qui vont bien sur l'archive pour le décompresser:
        if ext_fichier_2 == 'bin':
            print addr_archive
            os.system('chmod +x ' + addr_archive)
            os.system(addr_archive  + '  --prefix ' + config(info=2) + '/jeux/' + self.nom_jeu + ' --usergroup games --deskcut no --menu no --syswide no --installer-language fr --perms no --mode unattended')

        #Si c'est un installateur "InstallJammer" comme pour Regnum Online:
        if ext_fichier_2 == 'inj':
            os.system('chmod +x ' + addr_archive)
            os.system(addr_archive  + '  --prefix ' + config(info=2) + '/jeux/' + self.nom_jeu + ' --mode silent')

        #Si c'est un fichier .package, utilise un shell, avec l'option -x sur l'archive pour la décompresser:
        if ext_fichier_2 == 'package':
            os.system('bash ' + addr_archive + ' -x ' + config(info=2) + '/jeux')

        #Si c'est un fichier.run ('type installateur loki, utilise une commande shell pour décompresser
        if ext_fichier_2 == 'run':
            ...
            #Lancle l'installation dans un terminal,os.popen ne fonctionne pas chez certains utilisateurs
            if os.path.exists('/usr/bin/xterm') :
                os.popen('xterm -e sh ' + addr_archive + ' --target ' + config(info=2) + '/jeux/' +  self.nom_jeu +  ' --noexec')
            elif os.path.exists('/usr/bin/konsole'):
                os.popen('konsole -e sh ' + addr_archive + ' --target ' + config(info=2) + '/jeux/' +  self.nom_jeu +  ' --noexec')
            elif os.path.exists('/usr/bin/gnome-terminal'):
                os.popen('gnome-terminal -e sh ' + addr_archive + ' --target ' + config(info=2) + '/jeux/' +  self.nom_jeu +  ' --noexec')
            else:
                os.popen('sh ' + addr_archive + ' --target ' + config(info=2) + '/jeux/' +  self.nom_jeu +  ' --noexec')
                
            #Si le fichier utilise bz2, ce n'est pas une archive loki, donc on ne tente pas ce type d'installation
            if ext_fichier_1 == 'bz2' :
                try:
                    os.rename(config(info=2) + '/jeux/' + self.nom_jeu + '-tmp', config(info=2) + '/jeux/' + self.nom_jeu)
                except: pass
            else:
        ...
        #Le jeu est décompréssé/installé, on supprime ou pas l'archive en fonction de la configuration
    ...
    def loki_installe(self, nom_dossier):
        ...
        try:
            while boucle_fichiers < len(liste_binaires):
                try:
                    shutil.move(nom_dossier + '/bin/' + liste_binaires[boucle_fichiers], nom_dossier + '/')
                except:
                ...
                except OSError: print i18n.traduc_ascii("Le binaire du jeu n'a pas ete rendu executable suite a la levee d'une exception")
        ...
        while boucle_fichiers < len(liste_fichiers):
            ...
            if 'tar' in liste_fichiers[boucle_fichiers]:
                #On décompresse:
                fichier_tar = tarfile.open(config(info=2) + '/jeux/' + nom_dossier + '/' + liste_fichiers[boucle_fichiers],'r')
                fichier_tar.debug = 1 #Affiche les fichiers pendant la décompression:
        ...
        #On tente de supprimer les dossiers spécifiques aux installations loki (désormais inutiles)
        try:
...
class lance(threading.Thread):
    ...
    def run(self):
        ...
        #On supprime si il y a un espace à la fin (uniquement si il n'y a qu'une commande sans arguments):
        cmd_jeu_tmp = commande_jeu.split()
        if len(cmd_jeu_tmp) == 1:
            commande_jeu = cmd_jeu_tmp[0]

        #print '>'+str(commande_jeu) + '< >' + str(nom_jeu) + '< >' + str(variables.commande_jeu_p)
        ...
        if self.raccourci == 1:
            ...
            if self.dir_jeu != '' and self.dir_jeu != None:
            ...
            #Sinon, trouve le chemin du répertoire du jeu en fonction de la ligne de commande:
                ...
                #L'idée est de prendre le chemin vers le binaire (si il contient '/' et d'utiliser son répertoire parent.
            ...
            #Repère le répertoire du jeu, sinon bon nombre de jeux ne se lancent pas, puisqu'ils ne trouvent pas leurs données
        ...
        if int(config(info=8)) == 1:
            ...
            self.x = subprocess.Popen('X :' + str(no_x), executable=None, shell=True, cwd=home, stdout=self.log_x, stderr=self.log_x)
        ...
        if os.path.exists('/opt/lib32/lib'): #Répertoire des librairies 32 bits sur Archlinux
            ...
            LIBRARY_PATH = "" #Créé la chaine qui contiendra les répertoires contenant les dépendances:
        ...
        #Si l'on utilise le paquet de librairies (pour les dépendances) fourni par djl, on l'ajoute à la chaine LIBRARY_PATH:
        ...
        if type_exec == 'py':
            #os.chdir(chemin_jeu)
            #os.system('python ' + commande_j)
            self.p = subprocess.Popen('python ' + str(commande_j), executable=None, shell=True, cwd=chemin_jeu, stderr=self.fichier_log,  stdout=self.fichier_log)
        ...
        #Si c'est un script shell:
            ...
            self.p = subprocess.Popen(commande_j, executable=None, shell=True, cwd=chemin_jeu, stderr=self.fichier_log, stdout=self.fichier_log)
        ...
        #Sinon, on part du principe que c'est un binaire
            ...
            try:
                ...
                #print i18n.traduc_ascii("Le fichier n'a pas ete rendu executable") + ':',  commande_j
                pass
                
            #Si il y a d'autre fichiers binaires dans le répertoire, on les rend executables, (le script est suceptible de les utiliser)
            ...
            try:
                self.p=subprocess.Popen(commande_j, cwd=chemin_jeu, stderr=self.fichier_log, stdout=self.fichier_log)
                #Lance un timer qui analisera l'état du jeu, afin de repérer les anomalies:
                self.chrono()
            except:
                #try:
                self.p=subprocess.Popen(commande_j, cwd=chemin_jeu, executable=None, shell=True, stderr=self.fichier_log, stdout=self.fichier_log)
                self.chrono()
                #except:
    ...
    def retour(self):
        ...
        try:
            if int(config(info=8)) == 1 and self.x.poll() != None:
                self.log_x.close()
                variables.erreur=2
...
class msg_erreur():
    def __init__(self):
        ...
        if str(texte_log) != '':
            texte = i18n.traduc("Une erreur s'est produite avec ") +  variables.nom_jeu + i18n.traduc(". \nVoici le message d'erreur: \n")
        else:
            texte = _('Le jeu ') + variables.nom_jeu + i18n.traduc(" n'a put etre lance correctement. Merci de lire les eventuels messages d'erreur dans le terminal et rapporter le bogue au developpeur.")
        ...
        #Pour les jeux LGP
            ...
            if ".so" in texte_log:
                suggestion = "\n" + i18n.traduc("C'est probablement une incompatibilite entre librairies partages.")
            mesg = 1
            ...
            if "wrong ELF class: ELFCLASS" in texte_log:
                ...
                for i in range(len(liste_texte)):
                    if '.so' in liste_texte[i]:
                        ...
                        break
                if config(info=17) == 1:
                    suggestion = i18n.traduc("djl va tenter de trouver la librairie") + ': ' + nom_lib
                else:
            ...
            #Afin de suggérer à l'utilisateur de l'installer:
                ...
                if '.so' in str(texte_log):
                    ...
                    if config(info=17) == 1:
                        suggestion = i18n.traduc("djl va tenter de trouver la librairie") + ': ' + nom_lib.replace(':', '')
                    else:
        ...
        if mesg == 1:
            ...
            suggestion = suggestion + "\n\n" +i18n.traduc("Pour plus d'informations, vous pouvez consulter la sortie d'erreur du jeu dans le menu 'Information/Consulter' la sortie des jeux.")
            texte = message + "\n" + suggestion
