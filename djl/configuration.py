...
class Ui_Configuration(QtGui.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        if os.path.exists(home + '/.local/share/applications/djl.desktop') == True:
            ...
            self.b_raccourci.setText(
                i18n.traduc("Ajouter un raccourci de djl dans le menu KDE/Gnome"))

        self.b_parcourir.connect(self.b_parcourir, QtCore.SIGNAL("clicked()"), self.parcours_rep)
        self.b_parcourir_nav.connect(
            self.b_parcourir_nav,
    ...
    def valider(self):
        ...
        fichier_cfg = self.home + '/.djl/config'
        # Le si le fichier de configuration n'existe pas et que l'utilisateur
        # clique sur valider, on sauvegarde la configuration et on lance la
        # fenetre principale
    ...
    def ferme(self):
        ...
        fichier_cfg = self.home + '/.djl/config'
        # Le si le fichier de configuration n'existe pas et que l'utilisateur
        # clique sur fermer, on sauvegarde la configuration par défaut
        if not os.path.exists(fichier_cfg):
            self.sauve_config()
    ...
    def sauve_config(self):
        ...
        if not os.path.exists(self.adresse + '/etat_jeux'):
            try:
                # Créé le répertoir où seront créé les fichiers etat (pour savoir si les
                # jeux sont installés ou non dans l'interface)
        ...
        # if not os.path.exists(self.adresse + config(info=14)):
            ...
            # except:
                # print i18n.traduc_ascii("Erreur, le repertoire de definition n'a put
                # etre cree, vous n'avez peut etre pas les droits necessaires ou le
                # fichier est existant")
    ...
    def setupUi(self, Dialog):
        ...
        x, y = 470, 505
        Dialog.resize(QtCore.QSize(QtCore.QRect(200, 200, x, y).size())
    ...
    def retranslateUi(self, Dialog):
        ...
        self.check_b_icone_barre.setText(
            i18n.traduc("Iconifier djl dans la barre a miniature. (necessite redemarrage)"))
        ...
        self.check_b_dependances.setText(
            i18n.traduc("Installer automatiquement les dependances manquantes."))
